...
<ul class="<?php if ($row_danhmuc['id_danhmuc'] == $line['id_danhmuc']) { ?>sub-menu<?php } ?>">
    <?php
    while ($line = mysqli_fetch_array($row_xuat_danhmucphu)) {
        if ($row_danhmuc['id_danhmuc'] == $line['id_danhmuc']) {
    ?>
        <li>
            <a href="index.php?quanly=danhmucsanpham&id=<?php echo $line['id_danhmucphu'] ?>"><?php echo $line['tendanhmucphu'] ?></a>
        </li>
    <?php
        }
    }
